...
def call_numbers(numbers, boards):
    for call_number in numbers:
        for board in boards:
            ...
            bingo = horizontal_bingo or vertical_bingo

            if bingo:
                unmarked_nums_sum = num_board[~bool_board].sum().sum()
                return unmarked_nums_sum * call_number


print(f'Part 1 answer: {call_numbers(numbers_raw, boards)}')
